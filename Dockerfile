...
WORKDIR /yuriRolizEmissorMicroservico
...
COPY --from=build /yuriRolizEmissorMicroservico/target/*.jar yuriRolizEmissorMicroservico.jar

# Exponha a porta que a aplicação vai usar
EXPOSE 8081

# Comando para rodar a aplicação
ENTRYPOINT ["java", "-jar", "yuriRolizEmissorMicroservico.jar"]
